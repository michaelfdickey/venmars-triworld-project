<script lang="ts">
	import { venMarsPayloads, payloadCategoryLabels, payloadCategoryIcons, payloadInventory, cashBalanceM, type PayloadCategory, type PayloadDef } from '$lib/stores/gameStore';

	let { bodyId }: { bodyId: string } = $props();

	// Grouped by category for stacked display
	const categoryOrder: PayloadCategory[] = [
		'probe', 'weather', 'relay', 'nav',
		'terraforming', 'infrastructure', 'habitat',
		'vehicle', 'mining', 'supply', 'fuel',
	];
	...
	let filtered = $derived(
		selectedCategory === 'all'
			? venMarsPayloads
			: venMarsPayloads.filter(p => p.category === selectedCategory)
	);
	...
	// Categories with items for filter buttons
	let availableCategories = $derived(
		categoryOrder.filter(cat => venMarsPayloads.some(p => p.category === cat))
	);
...
<div class="venmars-tab">
	<div class="flex items-center justify-between mb-3">
		<div>
			<h3 class="text-base font-semibold">VenMars Payloads</h3>
			<p class="text-xs text-[var(--color-text-dim)]">Project-specific payloads for Venus & Mars operations</p>
		</div>
		<span class="text-xs text-[var(--color-text-dim)]">{venMarsPayloads.length} designs</span>
	</div>
	...
	<div class="filter-bar">
		...
		{#each availableCategories as cat}
			<button class="filter-btn" class:active={selectedCategory === cat} onclick={() => selectedCategory = cat}>
				{payloadCategoryIcons[cat]} {payloadCategoryLabels[cat]}
			</button>
		{/each}
	</div>
				...
				{#each group.items as payload}
					<button class="payload-card" onclick={() => detailPayload = payload}>
						<div class="payload-header">
							...
							{#if ($payloadInventory[payload.id] ?? 0) > 0}
								<span class="owned-badge">{$payloadInventory[payload.id]}</span>
							{/if}
						</div>
...
{#if detailPayload}
	<div class="modal-overlay" onclick={() => detailPayload = null} role="dialog" aria-modal="true">
		<div class="modal-content" onclick={(e) => e.stopPropagation()}>
			...
			{#if detailPayload.category === 'fuel'}
				<div class="detail-grid">
					<div class="detail-item">
						<span class="detail-label">Density</span>
						<span class="detail-value">{detailPayload.density_kg_m3} kg/m³</span>
					</div>
					<div class="detail-item">
...
<style>
	...
	.fuel-auto { color: #f59e0b; font-style: italic; }
	...
	.buy-btn:hover { background: rgba(59, 130, 246, 0.3); }
</style>
